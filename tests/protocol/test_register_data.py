...
def test_register_data(mocker: Callable[..., Generator[MockerFixture, None, None]]):
    ...
    mocker.patch(
        "sfkit.protocol.register_data.get_username", return_value="user@example.com"
    )
    mocker.patch("sfkit.protocol.register_data.update_firestore")
    mocker.patch(
        "sfkit.protocol.register_data.validate_sfgwas", return_value=("hi", "hello")
    )
    ...
    mocker.patch(
        "sfkit.protocol.register_data.get_doc_ref_dict",
        return_value=local_mock_doc_ref_dict,
    )
...
def test_encrypt_mpcgwas(mocker: Callable[..., Generator[MockerFixture, None, None]]):
    ...
    mocker.patch(
        "sfkit.protocol.register_data.encrypt_data", side_effect=Exception("error")
    )
    register_data.encrypt_mpcgwas("1", "MPC-GWAS")
...
def test_validate_sfgwas(mocker: Callable[..., Generator[MockerFixture, None, None]]):
    ...
    register_data.validate_sfgwas(
        mock_doc_ref_dict, "a@a.com", "data_path", "geno_binary_file_prefix"
    )
    register_data.validate_sfgwas(
        mock_doc_ref_dict, "a@a.com", "demo", "geno_binary_file_prefix"
    )
    ...
    register_data.validate_sfgwas(
        mock_doc_ref_dict_copy, "a@a.com", "data_path", "geno_binary_file_prefix"
    )
...
def test_validate_mpcgwas(mocker: Callable[..., Generator[MockerFixture, None, None]]):
    ...
    mocker.patch("sfkit.protocol.register_data.using_demo")
    mocker.patch(
        "sfkit.protocol.register_data.validate_mpcgwas_data", return_value=(1, 1)
    )
    mocker.patch("sfkit.protocol.register_data.condition_or_fail")
...
def test_validate_data_path(
    ...
    mocker.patch("sfkit.protocol.register_data.os.path.isabs", return_value=True)
    mocker.patch("os.path.exists", return_value=True)
    assert register_data.validate_data_path("") == "/data"
...
def test_validate_sfgwas_data(
    ...
    assert (
        register_data.validate_sfgwas_data("geno_binary_file_prefix%d", "data_path")
        == 1
    )
