...
SFKIT_DIR = os.environ.get(
    "SFKIT_DIR", os.path.join(os.path.expanduser("~"), ".config", "sfkit")
)
AUTH_FILE = os.path.join(SFKIT_DIR, "auth.txt")
AUTH_KEY = os.path.join(SFKIT_DIR, "auth_key.txt")
IS_DOCKER = os.path.exists("/.dockerenv")
IS_INSTALLED_VIA_SCRIPT = (
    is_installed("sfgwas") and is_installed("plink2") and is_installed("GwasClient")
) or TERRA_DEPLOYMENT_ENV
EXECUTABLES_PREFIX = (
    os.path.expanduser("~") + "/.local/" if IS_INSTALLED_VIA_SCRIPT else ""
)
if IS_INSTALLED_VIA_SCRIPT:
    ...
    if os.environ.get("PATH"):
        os.environ[
            "PATH"
        ] += f":{EXECUTABLES_PREFIX}bin:{EXECUTABLES_PREFIX}sfgwas:{EXECUTABLES_PREFIX}sf-relate:{EXECUTABLES_PREFIX}sf-skat:{EXECUTABLES_PREFIX}secure-gwas/code/bin"
...
SFKIT_PROXY_ON: bool = os.getenv("SFKIT_PROXY_ON", "true").lower() == "true"
SFKIT_PROXY_ARGS = os.getenv("SFKIT_PROXY_ARGS", "-v")
FORCE_QUIT_MAGIC = "force-quit-sfkit-protocol-magic-07102026"
